...
files.each do |file|
  ...
  if file_contents[1].nil? || file_contents[1].length > 200
    ...
    artist, album_title, release_date = file_contents[1].scan /(.+)\r\n(.+)\(?(.+)?\)?/
    if album_title && album_title.is_a?(String)
      album = Album.where(title: album_title.strip, release_year: release_date.try(:strip)).first_or_create
    end
    if file_contents[2]
      track_contents = file_contents[2].split(/Note Varie.*\r\n\r\n/)[0]
      track_array = track_contents.split("\r\n\r\n")
      ...
      track_details = track_array.pop
      
      _, artists, track_style_ext, ext_details, capo = track_details.split("\r\n")

      puts "#{artists.split("-").first.split(/,|y\W/).inspect}"
      puts "#{track_style_ext}"
      puts "#{ext_details}"
      puts "#{capo}"
      
      if capo !=~ /DD/
        fret, form = capo.scan(/Capotasto: (\d) in (.+) -/)
        puts "#{fret}, #{form}"
      end
